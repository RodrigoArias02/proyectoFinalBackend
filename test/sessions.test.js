...
describe("Prueba proyecto ecommerce", async function() {
    ...
    describe("Prueba modulo productos", async () => {
        ...
        it("La ruta api/sessions/current en su metodo POST deberia guardar la session en mongo", async () => {
            const usuario = {
                email: "RodrigoPremium@gmail.com",
                password: "1234",
              };
              let {body, status, ok } = await requester
        ...
        it("La ruta api/products en su metodo POST debería guardar un producto", async () => {
            ...
            // Verificar el estado de la respuesta y la redirección
            expect(request.status).to.be.equal(302);
            expect(request.header.location).to.equal(`/login?user=${usuario.email}`);
        
            // Verificar si el usuario se ha guardado correctamente en la base de datos
            let searchUser = await UserServices.getByEmail(usuario.email);
            expect(searchUser).to.be.an("object");

            let deleteUser= await UserServices.deleteUserService(usuario.email)
            console.log(deleteUser)
            expect(deleteUser.status).to.be.equal(201);
        });
        after(async () => {
            try {
